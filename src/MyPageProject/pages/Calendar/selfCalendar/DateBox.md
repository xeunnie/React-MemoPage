...
function DateBox({ nowDate, setNowDate, clickedDate, setClickedDate, holiday }: Props) {
  const allDay: Date[] = monthList(nowDate);

  const weeks = ['일', '월', '화', '수', '목', '금', '토'];

  const holidayLocDate = holiday.map((data: Holiday) => String(data?.locdate));

  return (
    <Container>
      {weeks.map((week) => (
        <WeekBox key={week} weekName={week} />
      ))}
      {allDay.map((day: Date) => {
        const yyyymmdd = dateToyyyymmdd(day);
        const todayIsHoliday = holidayLocDate.indexOf(yyyymmdd);
        const isHoliday = todayIsHoliday !== -1;
